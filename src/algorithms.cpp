...
bool Dinic::BreadthFirstSearch(int source, int sink) {
...
long long Dinic::DepthFirstSearch(int u, int sink, long long flow) {
    if (u == sink) {
        return flow;
    }
...
long long Dinic::MaxFlow(int source, int sink) {
    long long ans = 0;
    while (BreadthFirstSearch(source, sink)) {
        ...
        while (pushed > 0) {
            ans += pushed;
            pushed = DepthFirstSearch(source, sink, INF);
        }
    }
    return ans;
}
